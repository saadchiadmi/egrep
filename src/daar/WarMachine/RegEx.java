...
public class RegEx {
  ...
  public static void main(String arg[]) throws FileNotFoundException, IOException {
    ...
    if (regEx.length()<1) {
      ...
      }else if(!Util.isWord(regEx)){
          KMP.displayAllLinesContainsWordInFile(regEx, file);
          
      }else{
          if(!new File(Util.getFileName(file)+".index").isFile()){
              Indexing.forwardIndexing(file);
          }
          Util.displayAllLinesContaintWordInIndexFile(file, regEx);
      }
  ...
  private static Automate toAutomate(RegExTree ret) throws Exception {
	  switch (ret.root) {
	case RegEx.ETOILE:
		return Automate.etoileAuto(toAutomate(ret.subTrees.get(0)));
	
	case RegEx.CONCAT:
		return Automate.concatAuto(toAutomate(ret.subTrees.get(0)), toAutomate(ret.subTrees.get(1)));
